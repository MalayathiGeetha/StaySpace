/*
  ...
  1. New Tables
    - `listings`
      ...
      - `images` (text array, URLs to listing photos)
      - `host_name` (text, required)
      - `host_avatar` (text, optional URL)
      - `is_superhost` (boolean, default false)
      - `amenities` (text array, list of amenities)
      - `property_type` (text, required - Villa, Cabin, etc.)
      - `bedrooms` (integer, required)
      - `bathrooms` (integer, required)
      - `guests` (integer, max guest capacity)
  ...
  2. Security
    ...
    - Add policy for public to read all listings
    - Add policy for authenticated users to insert listings
    - Add policy for authenticated users to update listings

  3. Sample Data
    - Insert 6 sample listings with realistic data
    - Include various property types and locations
    - Mix of superhost and regular hosts
*/
